...
def compute_probabilities(X, theta, temp_parameter):
    ...
    Returns:
        H - k x n NumPy array, where each entry H[j][i] is the probability that X[i] is labeled as j
    """
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    Args:
        ...
        lambdaFactor - the regularization constant (scalar)
    ...
    non_regularized_grad = np.dot(M-probabilities,X)
    non_regularized_grad*=-item/num_examples
    return theta - alpha*(non_regularized_grad + lambda_factor*theta)
def my_func(x):
    return x%3
...
def update_y(train_y, test_y):
    ...
    train_y_mod3 = vecfunc(train_y)
    test_y_mod3 = vecfunc(test_y)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    vecfunc = np.vectorize(my_func)
    assigned_labels = vecfunc(prediction)
